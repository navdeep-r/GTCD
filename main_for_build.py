...
class GoogolCheatingDetectorApp(ctk.CTk):
    def __init__(self):
        ...
        # Show API key prompt before creating the UI
        self.show_api_key_popup()
        
    def show_api_key_popup(self):
        """Show popup to get Gemini API key from user"""
        popup = ctk.CTkToplevel(self)
        popup.title("Enter Gemini API Key")
        ...
        ctk.CTkLabel(popup, 
                    ...
                    font=("Arial Black", 20)).pack(pady=(20, 10))
        ...
        ctk.CTkButton(button_frame, 
                    text="Continue",
                    font=("Arial", 14),
                    command=lambda: self.confirm_api_key(popup)).pack(side="left", padx=10)
    ...
    def confirm_api_key(self, popup):
        """Validate and save the API key, then continue"""
        ...
        # Now create the UI and initialize
        self.create_ui()
        ...
        if self.init_gemini():
            # Initialize camera
            if self.init_camera():
                proctor = GoogolCheatingDetectorAI(self)
    ...
    def init_gemini(self):
        ...
        try:
            ...
            self.log_event("INFO", "Gemini API initialized successfully")
            return True
    ...
    def create_ui(self):
        ...
        self.api_key_button = ctk.CTkButton(control_frame, text="🔑 Change API Key",
                                         font=("Arial", 14), command=self.show_api_key_popup)
        self.api_key_button.pack(side="right", padx=10)
